...
def db_sync_data(errors: list[str] | None,
    ...
    by the database), and thus they must be identified by *identity_column*, and ommited from *sync_columns*,
    ...
    if curr_source_conn and curr_target_conn:
        ...
        sel_stmt: str = (f"SELECT {all_cols} FROM {source_table} "
        ...
        try:
            ...
            target_cursor.execute(statement=sel_stmt.replace("OFFSET @offset ROWS ", ""))
            ...
            while source_rows or target_rows:
                ...
                if source_offset and not source_rows:
                    source_cursor.execute(statement=sel_stmt.replace("@offset", str(source_offset)))
                ...
                if target_offset and not target_rows:
                    target_cursor.execute(statement=sel_stmt.replace("@offset", str(target_offset)))
