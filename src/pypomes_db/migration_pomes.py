...
def db_migrate_data(errors: list[str] | None,
    ...
    For Oracle databases, this only applies to *GENERATED ALWAYS* identity columns, not to
    the ones *GENERATED BY DEFAULT*. The latter should not be flagged and, if appropriate,
    omitted from *source_columns*.
    ...
    if identity_column and not err_msg and target_engine != "oracle":
        from . import db_pomes
        # obtain the maximum value inserted
        recs: list[tuple[int]] = db_pomes.db_select(errors=op_errors,
        ...
        if not op_errors:
            ...
            set_stmts: list[str] = []
            if target_engine == "postgres":
                set_stmts.append(f"SELECT setval("
                                 f"pg_get_serial_sequence('{target_table}', '{identity_column}'), "
                                 f"{seq_value})")
            elif target_engine == "sqlserver":
                set_stmts.append(f"SET IDENTITY_INSERT {target_table} OFF")
                set_stmts.append(f"DBCC CHECKIDENT ('{target_table}', RESEED, {seq_value}")
            # execute the commands
            for set_stmt in set_stmts:
                db_pomes.db_execute(errors=op_errors,
                                    exc_stmt=set_stmt,
                                    engine=target_engine,
                                    connection=target_conn,
