...
def db_migrate_data(errors: list[str] | None,
    ...
    The paramter *has_nulls* indicates that one or more columns holding string content in the source
    ...
    :param has_nulls: indicated that column holding strings in the source table has nulls
...
def db_stream_lobs(errors: list[str] | None,
    ...
    # buid the SELECT query
    source_pks: str = ", ".join(pk_columns)
    sel_stmt: str = f"SELECT {source_pks}, {lob_column} FROM {table}"
    if where_clause:
        sel_stmt += f" WHERE {where_clause}"
