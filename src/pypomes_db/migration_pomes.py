...
def db_migrate_data(errors: list[str] | None,
    ...
    The parameter *skip_rows* is used to offset the retrieval of tuples, whereas *batch_size_in* and
    *limit_rows* are used to limit the maximum number of tuples to retrieve in each SELECT operation,
    and *batch_size_out* indicates the maximum number of tuples to write in each INSERT operation.
    Both parameters *skip_rows* and *batch_size_in* are ignored if non-integers, or if less than 1.
...
def db_migrate_lobs(errors: list[str] | None,
    ...
    equivalent types, in both the origin and the destination databases.
    The parameter *skip_rows* is used to offset the retrieval of tuples, whereas *batch_size* and
    *limit_rows* are used to limit the maximum number of tuples to retrieve in each SELECT operation.
    Both parameters are ignored if non-integers, or if less than 1.
    The parameters *source_committable* and *target_committable* are relevant only
    if *source_conn* and *target_conn* are provided, respectively, and are otherwise ignored.
    A rollback is always attempted, if an error occurs.
